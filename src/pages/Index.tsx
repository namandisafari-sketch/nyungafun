...
const defaultContact = { phone: "0772 959 500, 0746 960 654", email: "info@nyungafoundation.com", location: "Katwe – Kampala, Byandala Nankya Complex, 1st Floor, Office No. 5", whatsapp: "0746 960 654" };

const defaultQuickLinks = [
  { title: "Our Programs", description: "Discover how we support students from nursery to university through our various programs.", linkText: "Learn more", linkTo: "/programs" },
  { title: "Partner Schools", description: "View the 50+ schools we partner with across Uganda to provide quality education.", linkText: "View schools", linkTo: "/schools" },
  { title: "About Us", description: "Learn about our mission, vision, values and the impact we've made in Ugandan communities.", linkText: "Read more", linkTo: "/about" },
];

const defaultScamWarning = {
  title: "⚠️ Beware of Scammers",
  body1: 'The Nyunga Foundation does NOT operate through any other websites, social media accounts, or agents online. We do NOT ask for money through mobile money, WhatsApp, or any online platform.',
  body2: 'If anyone contacts you claiming to represent Nyunga Foundation and asks for payment or personal information online, they are scammers. Please do not engage with them.',
  officialSite: "www.nyungafoundation.com",
};

const defaultClosingCta = {
  title: "Every Child Deserves a Chance",
  description: "Together, we can change the story for Uganda's most vulnerable children through the power of education.",
};

const quickLinkIcons = [Heart, BookOpen, Users];
...
const Index = () => {
  ...
  const qLinks = (settings?.quick_links as any[]) || defaultQuickLinks;
  const scam = settings?.scam_warning || defaultScamWarning;
  const cta = settings?.closing_cta || defaultClosingCta;
    ...
    <div className="min-h-screen flex flex-col">
        ...
        <div className="container mx-auto px-4">
          <div className="grid md:grid-cols-3 gap-8">
            {stats.map((stat: any, i: number) => {
              ...
              return (
                <div key={i} className="flex flex-col items-center text-center p-8 rounded-lg bg-muted/50">
      ...
      <section className="py-16 bg-muted/30">
        <div className="container mx-auto px-4 max-w-5xl">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {qLinks.map((link: any, i: number) => {
              const Icon = quickLinkIcons[i % quickLinkIcons.length];
              return (
                <Link key={i} to={link.linkTo || "/"} className={`group bg-card border border-border rounded-xl p-8 hover:shadow-lg hover:border-secondary/30 transition-all ${i === qLinks.length - 1 && qLinks.length % 2 !== 0 ? "sm:col-span-2 lg:col-span-1" : ""}`}>
                  <Icon size={32} className="text-secondary mb-4" />
                  <h3 className="font-display text-xl font-bold text-primary mb-2 group-hover:text-secondary transition-colors">{link.title}</h3>
                  <p className="text-muted-foreground text-sm leading-relaxed mb-4">{link.description}</p>
                  <span className="text-secondary text-sm font-semibold flex items-center gap-1 group-hover:gap-2 transition-all">{link.linkText || "Learn more"} <ArrowRight size={14} /></span>
                </Link>
              );
            })}
          </div>
      ...
      <section className="py-16 bg-destructive/10 border-y border-destructive/20">
        <div className="container mx-auto px-4 max-w-3xl">
          <div className="flex flex-col items-center text-center">
            ...
            <h2 className="font-display text-2xl md:text-3xl font-bold text-destructive mb-4">{scam.title}</h2>
            <p className="text-foreground leading-relaxed mb-4">{scam.body1}</p>
            <p className="text-foreground leading-relaxed mb-4">{scam.body2}</p>
            <div className="bg-card border border-border rounded-lg p-5 mt-4 w-full max-w-md">
              ...
              <ul className="text-sm text-muted-foreground space-y-1 text-left">
                <li className="flex items-center gap-2"><AlertTriangle size={14} className="text-secondary shrink-0" /> Website: <strong className="text-primary">{scam.officialSite || "www.nyungafoundation.com"}</strong></li>
      ...
      <section className="py-20 bg-primary">
        <div className="container mx-auto px-4 text-center">
          ...
          <h2 className="font-display text-3xl md:text-4xl font-bold text-primary-foreground mb-4">{cta.title}</h2>
          <p className="text-primary-foreground/80 max-w-md mx-auto text-lg">{cta.description}</p>
        </div>
      </section>
