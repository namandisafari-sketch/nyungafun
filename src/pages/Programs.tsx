import { GraduationCap, BookOpen, Heart, Users, Briefcase, Shield, Star, ArrowRight } from "lucide-react";
...
const programs = [
  {
    ...
    description: "We cover full or partial tuition fees for students from nursery to university level. Each beneficiary is carefully selected based on academic merit and financial need.",
    highlights: ["Nursery to University", "Merit-based selection", "Term-by-term disbursement"],
  },
  {
    icon: BookOpen,
    title: "Scholastic Materials",
    description: "Every term, we distribute exercise books, textbooks, mathematical sets, pens, and other essential learning materials to all our beneficiaries across partner schools.",
    highlights: ["Textbooks & exercise books", "Mathematical instruments", "Stationery packs"],
  },
  {
    icon: Shield,
    title: "School Uniforms & Essentials",
    description: "We provide school uniforms, shoes, bedding (for boarders), and other essentials that students need to attend school with dignity and confidence.",
    highlights: ["Uniforms & shoes", "Bedding for boarders", "Personal hygiene items"],
  },
  {
    icon: Users,
    title: "Mentorship & Guidance",
    description: "Beyond financial support, we provide mentorship programs that guide students through their academic journey, career choices, and personal development.",
    highlights: ["Academic counselling", "Career guidance", "Life skills training"],
  },
  {
    icon: Briefcase,
    title: "Examination Support",
    description: "We cover examination fees for national exams (PLE, UCE, UACE) ensuring that our beneficiaries can sit their exams without financial barriers.",
    highlights: ["PLE registration", "UCE & UACE fees", "Mock exam fees"],
  },
  {
    icon: Heart,
    title: "Vulnerability Support",
    description: "Special attention is given to orphans, children with disabilities, and those from extremely disadvantaged backgrounds. We provide additional psychosocial support and resources.",
    highlights: ["Orphan care", "Disability support", "Psychosocial counselling"],
  },
];
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="py-20 bg-primary text-primary-foreground">
        <div className="container mx-auto px-4 text-center max-w-3xl">
          ...
          <p className="text-primary-foreground/80 text-lg">
            Comprehensive support from nursery to university — because education is the most powerful tool for change.
          </p>
        </div>
      </section>
            ...
            {programs.map((program) => (
              <div key={program.title} className="bg-card border border-border rounded-xl p-7 hover:shadow-lg hover:border-secondary/30 transition-all group">
                <div className="w-12 h-12 rounded-lg bg-secondary/10 flex items-center justify-center mb-5 group-hover:bg-secondary/20 transition-colors">
                  <program.icon size={24} className="text-secondary" />
                </div>
              ...
              {educationLevels.map((item, idx) => (
                <div key={item.level} className="flex gap-6 items-start">
                  <div className="hidden md:flex flex-col items-center shrink-0">
                    <div className="w-12 h-12 rounded-full bg-secondary text-secondary-foreground flex items-center justify-center font-display font-bold text-sm z-10">
                      {idx + 1}
                    </div>
                  </div>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={item.step} className="text-center">
                <div className="w-14 h-14 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-display text-xl font-bold mx-auto mb-4">
                  {item.step}
                </div>
      ...
      <section className="py-20 bg-primary">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-primary-foreground/80 max-w-md mx-auto text-lg mb-8">
            Visit our office to learn about our programs and how you can support a child's education.
          </p>
          <div className="flex flex-wrap justify-center gap-4">
            <Link to="/schools">
              <button className="bg-secondary text-secondary-foreground hover:bg-secondary/90 font-semibold text-base px-8 py-3 rounded-md transition-colors flex items-center gap-2">
                View Partner Schools <ArrowRight size={16} />
              </button>
            </Link>
            <Link to="/about">
